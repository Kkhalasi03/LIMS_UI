...
@Component({
  ...
  styleUrl: './test-result-edit.component.css'
})
export class TestResultEditComponent {
  ...
  constructor(
    ...
    private apiService: TestResultService
  ) {}
...
groupTestsByService(data: { ServiceId: number; TestId: number }[]): { ServiceId: number; TestIds: number[] }[] {
  ...
  toggleServiceStatus(test: any): void {
    test.ServiceStatus = test.ServiceStatus === 'V' ? 'N' : 'V';
  }
 ...
 this.groupedTests.forEach(group => {
   ...
   group.TestResults.forEach((test: { TestId: number; ResultValue: string; ServiceStatus: string }) => {
     serviceTest.TestResults.push({
       ...
       ServiceStatus: test.ServiceStatus ? "V" : "N"  // Set status based on validation
     });
   });
 ...
 console.log("Sending Payload:", JSON.stringify(payload, null, 2));
    ...
    this.apiService.SaveTestResultDetails(payload).subscribe(
      (response) => {
  ...
  goBack(): void {
    this.router.navigate(['/TestResult/testresult-list']);
  }
